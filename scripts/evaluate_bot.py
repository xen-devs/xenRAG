...
from ai_core.graph.graph import build_graph  # noqa: E402
...
async def run_case(
    ...
    explanation_conf = float(getattr(top_explanation, "confidence", 0.0)) if top_explanation else None
    evidence_ids = list(getattr(top_explanation, "evidence_ids", [])) if top_explanation else []

    claim_support = metric_claim_support(answer, case.expected_keywords)
    ragas_factual_proxy = (
        text_similarity(answer, case.reference_answer) if case.reference_answer else None
    )
    aspect_coverage = metric_aspect_coverage(answer, case.expected_aspects)
    recall_at_k = metric_recall_at_k(retrieved_ids, case.gold_evidence_ids, top_k)

    evidence_grounding = None
    if evidence_ids:
        evidence_grounding = safe_div(
            len(set(evidence_ids) & set(retrieved_ids)),
    ...
    result = {
        ...
        "explanation_usefulness_0_to_5": to_0_5(explanation_conf),
    ...
    logger.info(
        ...
        case.case_id,
        top_k,
        f"{recall_at_k:.3f}" if recall_at_k is not None else "NA",
        f"{ragas_factual_proxy:.3f}" if ragas_factual_proxy is not None else "NA",
        f"{claim_support:.3f}" if claim_support is not None else "NA",
    )
    return result
...
def aggregate_results(case_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "explainability_metrics": {
            "claim_support_ratio": mean_or_none(claims),
            "explanation_usefulness_0_to_5": mean_or_none(explain),
        },
        "retrieval_metrics": {
            "recall_at_k": mean_or_none(recall),
            "ragas_factual_proxy": mean_or_none(factual),
            "evidence_grounding": mean_or_none(grounding),
            "aspect_coverage": mean_or_none(coverage),
        },
        "clarification_metrics": {
            "trigger_rate": clarification_trigger_rate,
            "success_rate": clarification_success_rate,
            "avg_turns": 1.0,  # Single-turn eval script.
        },
    }
